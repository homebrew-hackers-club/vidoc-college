...
**The risk:** Even if `express` itself is trustworthy, any of those additional packages could be compromised. The `event-stream` attack happened through a transitive dependency — most developers didn't even know they were using it <sup>[3](#3)</sup>.
