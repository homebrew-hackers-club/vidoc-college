...
![The Trust Paradox](https://raw.githubusercontent.com/homebrew-hackers-club/vidoc-college/refs/heads/main//assets/safe-false-security.png)
...
- **Perceived security increases** — Developers report feeling that AI-generated code is "more secure" or "as secure" as human-written code <sup>[1](#1)</sup>
- **Actual security decreases** — Empirical studies show AI-generated code contains more vulnerabilities and receives less scrutiny <sup>[2](#2)</sup>
- **Review quality drops** — Code reviewers spend less time and effort reviewing AI-assisted code, assuming the AI has "handled" security concerns <sup>[3](#3)</sup>
...
![False Security Assumptions](https://raw.githubusercontent.com/homebrew-hackers-club/vidoc-college/refs/heads/main//assets/safe-false-security-2.png)
...
const token = jwt.sign(
  ...
  { expiresIn: '30d' }  // ⚠️ 30 days is too long
);
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    ...
    req.user = user;  // ⚠️ Trust token data directly
    next();
  });
}
...
function requestPasswordReset(email) {
  const token = Math.random().toString(36).substring(7);  // ⚠️ Not cryptographically secure
...
app.post('/api/users/:id/role', authenticateToken, (req, res) => {
  const { role } = req.body;
  db.users.update(req.params.id, { role });  // ⚠️ No authorization check
...
WORKDIR /app
# ⚠️ Copying from builder doesn't verify checksums
COPY --from=builder /app/dist ./dist
COPY package*.json ./
# ⚠️ npm ci doesn't verify package integrity
RUN npm ci --production
# ⚠️ No health check user separation
EXPOSE 3000
# ⚠️ Running Node.js as root
CMD ["node", "dist/index.js"]
```
...
<Mermaid chart={`
xychart-beta
    title "Vulnerability Accumulation Over Time"
    x-axis [Month 1, Month 2, Month 3, Month 4, Month 5, Month 6]
    y-axis "% of Codebase with Vulnerabilities" 0 --> 100
    line [20, 36, 49, 59, 67, 74]
`} />
...
description="Examine your own assumptions 🤔" 
...
<a id="1">[1]</a> **Snyk Podcast (2025)** – [The AI Security Report](https://snyk.io/podcasts/the-secure-developer/the-ai-security-report/)

<a id="2">[2]</a> **arXiv (2023)** – [Do Users Write More Insecure Code with AI Assistants?](https://arxiv.org/abs/2211.03622)

<a id="3">[3]</a> **ScienceDirect (2021)** – [Can AI artifacts influence human cognition? The effects of artificial autonomy in intelligent personal assistants](https://www.sciencedirect.com/science/article/abs/pii/S0268401220314493)
