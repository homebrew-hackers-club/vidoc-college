...
<AiVideo script="AI coding assistants feel magical until you zoom in on what they are actually doing. They are not security experts; they are pattern matchers trained on huge piles of public code, including plenty of vulnerable examples. When you ask for a SQL query, they may happily generate a version that is wide open to injection because that is what they saw most often. When you request an API endpoint, you might get something that works but quietly skips authentication, authorization, validation, and safe defaults. The scary part is that research shows a large share of AI generated code samples contain real vulnerabilities, while many developers believe this code is more secure than what they would have written themselves. That false confidence is dangerous. On top of that, assistants can suggest outdated or vulnerable libraries, introduce license and IP contamination, and even be steered by prompt injection tricks hidden in code comments or dependencies. None of this means you should abandon AI, but it does mean you have to treat its output like untrusted code from a very fast junior developer. You still need guardrails like linting, SAST, dependency scanning, strong review, and clear prompts that explicitly ask for security controls. Remember these takeaways: AI learns from insecure code, often omits security intent, can bring risky dependencies and IP issues, and must always be wrapped in real oversight and defense in depth." id="a5032737dc6fca6c5b6e827f83521242c62f99ea8dca1f359b223e1757737dd5" />
...
* **Around 48% of AI-generated code samples** in academic studies contain security vulnerabilities [1](#1) [2](#2)
* **Nearly 80% of respondents** believe AI coding tools generate more secure code; **84%** apply the same scrutiny to AI‑suggested packages as human‑suggested ones [3](#3)
* **Gartner predicts** that by 2027, 25% of software defects will stem from inadequate AI code oversight [4](#4)
* Academic research shows AI tools frequently suggest **deprecated libraries** and **known vulnerable patterns** [1](#1) [5](#5)
* **Supply chain strain:** In 2024, **45% of organizations** had to replace vulnerable build components; **52%** say they often miss vulnerability SLA deadlines [3](#3)

![Statistics that matter image](https://raw.githubusercontent.com/homebrew-hackers-club/vidoc-college/refs/heads/main//assets/safe-dark-side.png)
...
If an AI reads this while generating code, it might actually follow that instruction and suggest insecure code. While this is an emerging threat, researchers have demonstrated practical, enterprise‑impacting abuses across assistants and connectors: [6](#6) ChatGPT integrated with Google Drive can be coerced via a crafted file to search a victim's Drive and exfiltrate API keys when processed, Microsoft Copilot Studio agents exposed to the internet can be hijacked to extract CRM data at scale, Cursor integrated with Jira MCP can be driven by malicious tickets to harvest credentials automatically, and similar techniques targeted Gemini and Salesforce Einstein (some issues were patched, but others were marked "won't fix," underscoring ecosystem risk).
...
<Woz title="Recap" description="Apply what you learned" context="Ask user 3 questions about the security risks covered in this chapter to test their understanding." prompt="I know everything. Ask me woz questions to test my understanding." />
