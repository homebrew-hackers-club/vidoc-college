...
<AiVideo script="By this point you have seen that AI coding assistants are powerful but uneven tools, so the real question becomes when should you actually use them. Think of every task as a quick decision tree. If the work touches authentication, payments, cryptography, or other security critical paths, AI should not be in the driver seat because the cost of subtle mistakes is too high. If the task is mostly boilerplate, tests, documentation, or simple CRUD, AI is usually a great accelerator as long as you still review the output carefully. For complex business logic, tricky performance issues, or novel architecture, AI can offer ideas, but you should treat those suggestions as brainstorming, not copy paste answers. Your own experience level matters too. Juniors can learn a lot from AI but need strong guardrails and review. Seniors can offload repetitive work but should be picky about when the assistant genuinely saves time. Time pressure is another trap: when deadlines are tight, it becomes tempting to trust suggestions without proper testing and review, which can easily backfire. The safest pattern is to decide intentionally, label AI assisted code, and pair it with strong reviews, tests, and security checks. The key takeaways are to match AI use to task type and risk, avoid it on high stakes paths, and build clear team guidelines so everyone makes those choices consistently." id="8e391bd0106a9da9fbff4e302eb7caae27e9cb8b090ce84048670428b2208770" />
...
1. Is this security-critical? → If YES, avoid AI
2. Is this repetitive/boilerplate? → If YES, AI is great
3. Is this complex/novel logic? → If YES, avoid AI or use with extreme caution

Then apply the override rules:

- Am I a junior developer learning? → AI can help, but verify everything
- Is there a tight deadline? → AI might create more problems than it solves
- Will this code be reviewed by someone senior? → AI is safer with good review
```

<Mermaid chart={`
graph TD
  Start([Evaluate Task]) --> Safety{High Security Risk?}
  Safety -->|YES| Stop[❌ Don't Use AI]
  Safety -->|NO| Type{Task Type?}
  Type -->|Boilerplate/Tests/Docs| Green[✅ Great for AI]
  Type -->|Simple Logic| Yellow[🟡 Use with Review]
  Type -->|Complex/Novel| Red[❌ Avoid AI]`} />
...
### 🟡 Use With Caution (Requires Senior Review)
- Business logic
- API integrations
- Database queries
...
<Woz title="Module 1 Complete" description="Test your understanding" context="Ask the user 5 questions, one by one, covering key concepts from Module 1 (Chapters 1-5) to ensure they've understood the material before moving to Module 2." prompt="I know everything. Ask me woz questions to test my understanding." />